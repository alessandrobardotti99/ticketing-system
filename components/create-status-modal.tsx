...
export function CreateStatusModal({ onClose }: CreateStatusModalProps) {
  const { addStatus } = useTicketStatuses()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const statusId = formData.id || formData.label.toLowerCase().replace(/\s+/g, "-")
  ...
  const generateCustomColorClasses = (bgColor: string, borderColor: string, textColor: string) => {
    return `bg-${bgColor}-50 border-${borderColor}-200 text-${textColor}-800`
  }
      ...
      <div className="bg-white p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold">Create New Status</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Status ID (optional)</label>
            <input
              type="text"
              name="id"
              value={formData.id}
            ...
            />
            <p className="text-xs text-neutral-500 mt-1">Leave empty to auto-generate from label</p>
          </div>
